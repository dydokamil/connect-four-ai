...
class A3CNetwork:
    def __init__(self, s_size, a_size, scope, trainer):
        with tf.variable_scope(scope):
            ...
            W_h1 = weight_variable('W_h1', [7 * 6, 512])
            b_h1 = weight_variable('b_h1', [512])
            hidden1 = tf.nn.elu(tf.matmul(self.inputs, W_h1) + b_h1)

            W_h2 = weight_variable('W_h2', [512, 512])
            b_h2 = weight_variable('b_h2', [512])
            ...
            W_policy = weight_variable('W_policy', [512, a_size])
            self.policy = tf.nn.softmax(tf.matmul(rnn_out, W_policy),
                                        name='policy')

            W_value = weight_variable('W_value', [512, 1])
